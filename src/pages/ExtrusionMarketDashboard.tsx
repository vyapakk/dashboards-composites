...
const extrusionYears = Array.from({ length: 2034 - 2018 + 1 }, (_, i) => 2018 + i);

const ExtrusionMarketDashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState(2025);
  const [activeTab, setActiveTab] = useState<MainTabType>("overview");
  const { data: marketData, isLoading, error, refetch } = useMarketData("/data/aircraft-interiors-extrusion-market.json");
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen">
      <ScrollToTop />
      <AircraftInteriorsDashboardHeader title="Aircraft Interiors Extrusion Market" subtitle="Global Market Research Dashboard • 2018-2034" />
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-12 border-t border-border pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left">
            <div>
              <p className="text-sm text-muted-foreground">Aircraft Interiors Extrusion Market Research Report</p>
...
export default ExtrusionMarketDashboard;
