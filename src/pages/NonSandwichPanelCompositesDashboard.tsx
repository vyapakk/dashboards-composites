...
import { AlertCircle, RefreshCw, ArrowLeft, BarChart3, Plane, Globe, Users, Layers, Package, FlaskConical } from "lucide-react";
...
const nonSandwichTabs: TabConfig[] = [
...
const nonSandwichYears = Array.from({ length: 2034 - 2016 + 1 }, (_, i) => 2016 + i);

const NonSandwichPanelCompositesDashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState(2025);
  const [activeTab, setActiveTab] = useState<MainTabType>("overview");
  const { data: marketData, isLoading, error, refetch } = useMarketData("/data/aircraft-interior-non-sandwich-panel-composites-market.json");
  ...
  const getSegmentInfo = () => {
    switch (activeTab) {
      ...
      case "process": return { data: marketData.processType || [], title: "Resin Type" };
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen">
      <ScrollToTop />
      <AircraftInteriorsDashboardHeader title="Aircraft Interior Non-Sandwich Panel Composites Market" subtitle="Global Market Research Dashboard • 2016-2034" />
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <MainNavigation
            ...
            onYearChange={setSelectedYear}
            showYearSelector
            tabs={nonSandwichTabs}
            years={nonSandwichYears}
          />
        </div>
        ...
        <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-12 border-t border-border pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left">
            <div>
              <p className="text-sm text-muted-foreground">Aircraft Interior Non-Sandwich Panel Composites Market Research Report</p>
...
export default NonSandwichPanelCompositesDashboard;
