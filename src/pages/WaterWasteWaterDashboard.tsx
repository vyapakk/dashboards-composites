...
import { AlertCircle, RefreshCw, ArrowLeft, BarChart3, Plane, Globe, Droplets, Users, Package, Wrench } from "lucide-react";
...
const waterWasteTabs: TabConfig[] = [
...
const waterWasteYears = Array.from({ length: 2034 - 2016 + 1 }, (_, i) => 2016 + i);

const WaterWasteWaterDashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState(2025);
  const [activeTab, setActiveTab] = useState<MainTabType>("overview");
  const { data: marketData, isLoading, error, refetch } = useMarketData("/data/aircraft-water-waste-water-market.json");
  ...
  const getSegmentInfo = () => {
    switch (activeTab) {
      ...
      case "application": return { data: marketData.application, title: "Water System Type" };
      case "equipment": return { data: marketData.furnishedEquipment, title: "Equipment Type" };
      case "process": return { data: marketData.processType || [], title: "Component Type" };
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen">
      <ScrollToTop />
      <AircraftInteriorsDashboardHeader title="Aircraft Water/Waste Water Market" subtitle="Global Market Research Dashboard • 2016-2034" />
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-12 border-t border-border pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left">
            <div>
              <p className="text-sm text-muted-foreground">Aircraft Water/Waste Water Market Research Report</p>
