...
import { AlertCircle, RefreshCw, ArrowLeft, BarChart3, Plane, Globe, Users, Package, Layers, Wrench, Box, FlaskConical } from "lucide-react";
...
const pottedInsertsTabs: TabConfig[] = [
...
const pottedInsertsYears = Array.from({ length: 2034 - 2017 + 1 }, (_, i) => 2017 + i);

const PottedInsertsMarketDashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState(2025);
  const [activeTab, setActiveTab] = useState<MainTabType>("overview");
  const { data: marketData, isLoading, error, refetch } = useMarketData("/data/aircraft-potted-inserts-market.json");
  ...
  const getSegmentInfo = () => {
    switch (activeTab) {
      case "endUser": return { data: marketData.endUser, title: "Sales Channel Type" };
      ...
      case "equipment": return { data: marketData.furnishedEquipment, title: "Core Material Type" };
      case "process": return { data: marketData.processType || [], title: "Fastener Type" };
      case "material": return { data: marketData.materialType || [], title: "Material Type" };
      default: return { data: marketData.endUser, title: "Sales Channel Type" };
    }
  };
  ...
  const renderTabContent = () => {
    if (activeTab === "overview") {
      return (
        <MarketOverviewTab
          ...
          endUserLabel="Sales Channel Type"
          equipmentLabel="Core Material Type"
          useMillions
        />
      );
    }
    ...
    return (
      <SegmentDetailTab
        ...
        endUserLabel="Sales Channel Type"
        equipmentLabel="Core Material Type"
        processTypeLabel="Fastener Type"
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen">
      <ScrollToTop />
      <AircraftInteriorsDashboardHeader title="Aircraft Potted Inserts Market" subtitle="Global Market Research Dashboard • 2017-2034" />
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <MainNavigation
            ...
            onYearChange={setSelectedYear}
            showYearSelector
            tabs={pottedInsertsTabs}
            years={pottedInsertsYears}
          />
        </div>
        ...
        <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-12 border-t border-border pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left">
            <div>
              <p className="text-sm text-muted-foreground">Aircraft Potted Inserts Market Research Report</p>
...
export default PottedInsertsMarketDashboard;
