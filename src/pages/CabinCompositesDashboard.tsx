...
import { AlertCircle, RefreshCw, ArrowLeft, BarChart3, Plane, Globe, Layers, Users, Beaker, Cog } from "lucide-react";
...
const compositesTabs: TabConfig[] = [
  { id: "overview", label: "Market Overview", icon: BarChart3 },
  { id: "segment2", label: "Aircraft Type", icon: Plane },
  { id: "region", label: "Region", icon: Globe },
  { id: "segment3", label: "Application", icon: Layers },
  { id: "segment1", label: "Sales Channel", icon: Users },
  { id: "segment4", label: "Composites Type", icon: Beaker },
  { id: "segment5", label: "Process Type", icon: Cog },
];

const CabinCompositesDashboard = () => {
  ...
  const getSegmentInfo = () => {
    switch (activeTab) {
      case "segment1": return { data: marketData.segment1, title: "Sales Channel" };
      ...
      case "segment4": return { data: marketData.segment4, title: "Composites Type" };
      case "segment5": return { data: marketData.segment5 || [], title: "Process Type" };
      default: return { data: marketData.segment1, title: "Sales Channel" };
    }
  };
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8"><MainNavigation value={activeTab} onChange={setActiveTab} selectedYear={selectedYear} onYearChange={setSelectedYear} showYearSelector tabs={compositesTabs} /></div>
        {renderTabContent()}
        <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-12 border-t border-border pt-6"><div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left"><div><p className="text-sm text-muted-foreground">Aircraft Cabin Interior Composites Market Research Report</p><p className="text-xs text-muted-foreground/70">All values in US$ Million unless otherwise specified</p></div><img src={stratviewLogoWhite} alt="Stratview Research" className="h-10 w-auto" /></div></motion.footer>
      </main>
    </div>
  );
};

export default CabinCompositesDashboard;
