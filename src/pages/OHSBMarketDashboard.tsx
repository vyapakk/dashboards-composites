...
import { AlertCircle, RefreshCw, ArrowLeft, BarChart3, Plane, Globe, Users, Package, BoxSelect } from "lucide-react";
...
const ohsbYears = Array.from({ length: 2034 - 2016 + 1 }, (_, i) => 2016 + i);

const OHSBMarketDashboard = () => {
  const navigate = useNavigate();
  const [selectedYear, setSelectedYear] = useState(2025);
  const [activeTab, setActiveTab] = useState<MainTabType>("overview");
  const { data: marketData, isLoading, error, refetch } = useMarketData("/data/aircraft-ohsb-market.json");
  ...
  const getSegmentInfo = () => {
    switch (activeTab) {
      ...
      case "application": return { data: marketData.application, title: "Stowage Bin Type" };
  ...
  const renderTabContent = () => {
    ...
    return (
      <SegmentDetailTab
        ...
        endUserLabel="End-User Type"
        equipmentLabel="Furnished Equipment Type"
        applicationLabel="Stowage Bin Type"
        useMillions
      />
    );
  };
  ...
  return (
    <div className="aircraft-interiors-theme min-h-screen">
      <ScrollToTop />
      <AircraftInteriorsDashboardHeader title="Aircraft OHSB Market" subtitle="Global Market Research Dashboard • 2016-2034" />
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-12 border-t border-border pt-6">
          <div className="flex flex-col items-center justify-between gap-4 text-center md:flex-row md:text-left">
            <div>
              <p className="text-sm text-muted-foreground">Aircraft OHSB Market Research Report</p>
...
export default OHSBMarketDashboard;
